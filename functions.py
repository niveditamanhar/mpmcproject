...
def create_post(self, nome, cpf, senha):
  ...
  try:        
    to_database = '{"Name": 'f'{json.dumps(nome)}'', "CPF" : 'f'{json.dumps(cpf)}'', "Password" : 'f'{json.dumps(senha)}''}'

    
    if nome == "":
      self.ids.lbregister.text = "Enter Name"

    elif cpf == "":
      self.ids.lbregister.text = "Enter CPF"

    elif senha == "":
      self.ids.lbregister.text = "Enter Password"  

    elif len(cpf) < 11:
      self.ids.lbregister.text = "CPF Inavlid"
      ...
      self.ids.lbregister.text = "Resitration Complete! Rediricting to Login"
...
def get_post(self, cpf, senha):
  ...
  if (cpf != '' and senha != ''):
    if (len(cpf) == 11 and cpf in res) and (len(senha) >= 10 and senha in res):
      self.ids.lblogin.text = "Login sucess!"                
    ...
    if(cpf == ''):
      self.ids.lblogin.text = "Enter CPF"

    elif(senha == ''):
      self.ids.lblogin.text = "Enter password"


    elif (cpf == '' and senha == ''):
      self.ids.lblogin.text = "Enter data to login"     
...
def redf_passwd(self, cpf, senha):
  ...
  if cpf == "":
    self.ids.lbredfsenha.text = "Enter a CPF"
  elif cpf not in res:
    self.ids.lbredfsenha.text = "CPF no"
  elif len(cpf) < 11:
    self.ids.lbredfsenha.text = "CPF Inválid"
  elif senha == "": 
    self.ids.lbredfsenha.text = "Enter a new password"
  elif len(senha) < 10:
    self.ids.lbredfsenha.text = "Password must be 10 characters"
    ...
    for usuario in user.each():
      if usuario.val()['CPF'] == f'{cpf}' :
        db.child("Users").child(usuario.key()).update({'Password': f'{senha}'})
        a = open(f'{cpf}.txt', "r")
    ...
    self.ids.lbredfsenha.text = "Password changed sucessfully!"
...
def change_screen(self, nome, cpf, id):
  ...
  user = db.child("Users").get()
  for usuario in user.each():
    if usuario.val()['Name'] in nome2:
      if f'{nome}' != "":
        db.child("Users").child(usuario.key()).update({'Name': f'{nome}'})
        ...
        f.close()
        self.ids.lbchange.text = "Name change sucess"
    ...
    if usuario.val()['CPF'] in cpf2:
      if f'{cpf}' != "":
        ...
        a.writelines(list_of_lines) 
        a.close()
        self.ids.lbchange.text = "CPFchange sucess"
        

    if usuario.val()['Password'] in id2:
      if f'{id}' != "":
        db.child("Users").child(usuario.key()).update({'Password': f'{id}'})
        ...
        b.close()
        self.ids.lbchange.text = "ID alter sucess!"

    self.ids.lbchange.text = "Data alter sucess!"
